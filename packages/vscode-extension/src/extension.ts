...
function toNonNegativeInt(value: unknown): number | null {
  ...
  const n = typeof value === 'number' ? value : numericValue;
...
type CodeLensResolveResult =
  | { ok: true; result: Awaited<ReturnType<typeof resolveFromArgs>> }
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand(
      ...
      async (uriStringArg?: unknown, rangeArg?: unknown) => {
        ...
        if (typeof uriStringArg === 'string' && rangeArg != null) {
          const attempt = await tryCodeLensResolve(uriStringArg, rangeArg);
          if (!attempt.ok) {
            vscode.window.showWarningMessage(attempt.error);
            return;
          }
