...
| v0.5 | Jun 2026 | Dependency security pass to clear the CI `npm audit --audit-level=high` gate: resolved 3 high-severity advisories transitive under `@vscode/vsce` (fast-uri 3.1.0→3.1.2, form-data 4.0.5→4.0.6, tmp 0.2.5→0.2.7) plus moderate bumps (qs, markdown-it, undici, uuid/@azure/msal-node) via lockfile-only `npm audit fix`; bumped esbuild 0.25→0.28.1 (Dependabot #24, bundle verified) — closed all 7 Dependabot PRs; corrected `PyCellParser` doc to AST-based (`@lezer/python`); 102 tests |
